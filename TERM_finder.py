...
def abrir_enlace():
    ...
    match = re.search(r'https://.*$', linea_completa)
...
def buscar_y_mostrar():
    search_phrase = f'"{entrada_busqueda.get()}"'

    resultados = []
    for resultado in search(search_phrase, num_results=30, advanced=True):
